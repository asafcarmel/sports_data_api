module SportsDataApi
  module Nba
    class Player
      attr_reader :stats, :season_stats
      def initialize(xml)
        if xml.is_a? Nokogiri::XML::Element
          ...
          season_stats_xml = xml.xpath('overall/total')
          if season_stats_xml.is_a? Nokogiri::XML::NodeSet and season_stats_xml.count > 0
            @season_stats = SportsDataApi::Stats.new(season_stats_xml.first)
          end
